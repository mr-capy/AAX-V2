...
AddEventHandler('KorioZ-PersonalMenu:Admin_giveCash', function(money)
	...
	if isAuthorized(getAdminCommand('givemoney'), plyGroup) then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, 'You Give ' .. money .. '$')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Admin_giveBank', function(money)
	...
	if isAuthorized(getAdminCommand('givebank'), plyGroup) then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, 'You Give ' .. money .. '$ in the bank')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Admin_giveDirtyMoney', function(money)
	...
	if isAuthorized(getAdminCommand('givedirtymoney'), plyGroup) then
		...
		TriggerClientEvent('esx:showNotification', xPlayer.source, 'You Give ' .. money .. '$ sale')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_promouvoirplayer', function(target)
	...
	if (targetXPlayer.job.grade == tonumber(getMaximumGrade(sourceXPlayer.job.name)) - 1) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You must request permission from the ~ r ~ Government ~ w ~.')
	else
		if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
			...
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You have ~ g ~ promoted ' .. targetXPlayer.name .. '~w~.')
			TriggerClientEvent('esx:showNotification', target, 'You have been ~ g ~ promoted by ' .. sourceXPlayer.name .. '~w~.')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You do not have ~ r ~ ~ w ~ permission.')
		end
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_destituerplayer', function(target)
	...
	if (targetXPlayer.job.grade == 0) then
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You cannot ~ r ~ downgrade ~ w ~ any further.')
	else
		if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
			targetXPlayer.setJob(targetXPlayer.job.name, tonumber(targetXPlayer.job.grade) - 1)

			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You have ~ r ~ demoted ' .. targetXPlayer.name .. '~w~.')
			TriggerClientEvent('esx:showNotification', target, 'You have been ~ r ~ demoted by ' .. sourceXPlayer.name .. '~w~.')
		else
			TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You do not have ~ r ~ ~ w ~ permission.')
		end
	end
...
AddEventHandler('KorioZ-PersonalMenu:Boss_recruterplayer', function(target, job, grade)
	...
	if sourceXPlayer.job.grade_name == 'boss' then
		...
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You have ~ g ~ recruited ' .. targetXPlayer.name .. '~w~.')
		TriggerClientEvent('esx:showNotification', target, 'You were ~ g ~ hired by ' .. sourceXPlayer.name .. '~w~.')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_virerplayer', function(target)
	...
	if sourceXPlayer.job.grade_name == 'boss' and sourceXPlayer.job.name == targetXPlayer.job.name then
		...
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You have ~ r ~ fired ' .. targetXPlayer.name .. '~w~.')
		TriggerClientEvent('esx:showNotification', target, 'You have been ~ g ~ fired by ' .. sourceXPlayer.name .. '~w~.')
	else
		TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'You do not have ~ r ~ ~ w ~ permission.')
	end
end)
...
AddEventHandler('KorioZ-PersonalMenu:Boss_destituerplayer2', function(target)
	...
	if (targetXPlayer.job2.grade == 0) then
		TriggerClientEvent('esx:showNotification', _source, 'You cannot ~ r ~ downgrade ~ w ~ any further.')
	else
		if sourceXPlayer.job2.grade_name == 'boss' and sourceXPlayer.job2.name == targetXPlayer.job2.name then
			targetXPlayer.setJob2(targetXPlayer.job2.name, tonumber(targetXPlayer.job2.grade) - 1)
