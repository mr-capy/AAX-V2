...
Citizen.CreateThread(function()
    while isLoading == true do 
        ...
        if ESX.IsPlayerLoaded(PlayerId) and playerData.job.name == "police" then 
            ...
            TriggerServerEvent("esx_policejob:getActivePlates")
            isLoading = false
        end
    end 
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == data.plate then 
            ...
            if nCheck >= Config.maxTracker then 
                ...
                SetNuiFocus(false)
                TriggerServerEvent("esx_policejob", data.plate)
            end
        else 
            miss = miss + 1 
        end 
...
RegisterNUICallback("removeSearch", function(data, cb)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == data.plate then 
            TriggerServerEvent("esx_policejob:removeActivePlate", data.plate)
            SendNUIMessage({
                type = "ui",
                display = false
              })
