...
ESX.RegisterUsableItem('turtlebait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message_fishing', _source, "~g~You attach the turtle bait onto your fishing rod")
	else
		TriggerClientEvent('fishing:message_fishing', _source, "~r~You dont have a fishing rod")
	end
...
ESX.RegisterUsableItem('fishbait', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message_fishing', _source, "~g~You attach the fish bait onto your fishing rod")
...
ESX.RegisterUsableItem('turtle', function(source)
	...
	if xPlayer.getInventoryItem('fishingrod').count > 0 then
		...
		TriggerClientEvent('fishing:message_fishing', _source, "~g~You attach the turtle meat onto the fishing rod")
	else
		TriggerClientEvent('fishing:message_fishing', _source, "~r~You dont have a fishing rod")
	end
...
RegisterNetEvent('fishing:catch_fishing')
AddEventHandler('fishing:catch_fishing', function(bait)
	...
	if bait == "turtle" then
		if rnd >= 78 then
			if rnd >= 94 then
				...
				TriggerClientEvent('fishing:message_fishing', _source, "~r~It was huge and it broke your fishing rod!")
				...
				if xPlayer.getInventoryItem('turtle').count > 4 then
					TriggerClientEvent('fishing:message_fishing', _source, "~r~You cant hold more turtles")
				else
					TriggerClientEvent('fishing:message_fishing', _source, "~g~You caught a turtle\n~r~These are endangered species and are illegal to posses")
			...
			if rnd >= 75 then
				if xPlayer.getInventoryItem('fish').count > 100 then
					TriggerClientEvent('fishing:message_fishing', _source, "~r~You cant hold more fish")
				else
					weight = math.random(4,9)
					TriggerClientEvent('fishing:message_fishing', _source, "~g~You caught a fish: ~y~~h~" .. weight .. "kg")
		...
		if bait == "none" then
			...
			if rnd >= 70 then
			TriggerClientEvent('fishing:message_fishing', _source, "~y~You are currently fishing without any equipped bait")
				if  xPlayer.getInventoryItem('fish').count > 100 then
						TriggerClientEvent('fishing:message_fishing', _source, "~r~You cant hold more fish")
					else
			...
			if rnd >= 82 then
						...
						if rnd >= 91 then
							...
							if xPlayer.getInventoryItem('shark').count > 0  then
									TriggerClientEvent('fishing:setbait', _source, "none")
									TriggerClientEvent('fishing:message_fishing', _source, "~r~You cant hold more sharks")
							else
									TriggerClientEvent('fishing:message_fishing', _source, "~g~You caught a shark!\n~r~These are endangered species and are illegal to posses")
									TriggerClientEvent('fishing:spawnPed_fishing', _source)
...
RegisterServerEvent("fishing:lowmoney_fishing")
AddEventHandler("fishing:lowmoney_fishing", function(money)
    local _source = source	
	local xPlayer = ESX.GetPlayerFromId(_source)
	xPlayer.removeMoney(money)
end)

RegisterServerEvent('fishing:startSelling_fishing')
AddEventHandler('fishing:startSelling_fishing', function(item)
