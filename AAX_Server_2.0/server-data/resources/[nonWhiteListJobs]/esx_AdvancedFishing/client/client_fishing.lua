...
Citizen.CreateThread(function()
	while true do
		...
		if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					TriggerServerEvent('fishing:catch_fishing', bait)
				else
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.SellFish.x, Config.SellFish.y, Config.SellFish.z, true) <= 3 then
			TriggerServerEvent('fishing:startSelling_fishing', "fish")
			Citizen.Wait(4000)
		end
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.SellShark.x, Config.SellShark.y, Config.SellShark.z, true) <= 3 then
			TriggerServerEvent('fishing:startSelling_fishing', "shark")
			Citizen.Wait(4000)
		end
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z, true) <= 3 then
			TriggerServerEvent('fishing:startSelling_fishing', "turtle")
			Citizen.Wait(4000)
		end
...
RegisterNetEvent('fishing:message_fishing')
AddEventHandler('fishing:message_fishing', function(message)
...
RegisterNetEvent('fishing:spawnPed_fishing')
AddEventHandler('fishing:spawnPed_fishing', function()
...
function OpenBoatsMenu(x, y , z)
	...
	if data.current.value == 'boat' then
		...
		TriggerServerEvent("fishing:lowmoney_fishing", 2500) 
	...
	if data.current.value == 'boat2' then
		...
		TriggerServerEvent("fishing:lowmoney_fishing", 5500) 
	...
	if data.current.value == 'boat3' then
		...
		TriggerServerEvent("fishing:lowmoney_fishing", 6000) 
	...
	if data.current.value == 'boat4' then
		...
		TriggerServerEvent("fishing:lowmoney_fishing", 7500) 
	...
	if data.current.value == 'boat5' then
		...
		TriggerServerEvent("fishing:lowmoney_fishing", 4500) 
	...
	if data.current.value == 'boat6' then
		...
		TriggerServerEvent("fishing:lowmoney_fishing", 3500) 
