...
function Spawn(Pcoords, Pped)	
	ESX.Game.SpawnVehicle(-2076478498, Pcoords, 13, function(spawnedVehicle)
		-- save & set plate
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.zonesfarm.farm.coords, true) < 50 then
			SpawnProps()
			Citizen.Wait(500)
		else
			Citizen.Wait(500)
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.zonesfarm.process.coords, true) < 1 then
...
function Process()
	...
	while timeLeft > 0 do
		Citizen.Wait(700)
		timeLeft = timeLeft - 1
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.zonesfarm.process.coords, false) > 4 then
...
Citizen.CreateThread(function()
	while true do
		...
		if nearbyObject then
					if not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR2" then
						ESX.ShowHelpNotification("Press ~INPUT_THROW_GRENADE~ to ~g~Harvest")
					end

					if IsControlJustReleased(0, 58) and not isPickingUp and GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "TRACTOR2" then
			...
			exports['progressBars']:startUI(2000, "Farming is being done ....")
	--[[
			TriggerEvent("mythic_progbar:client:progress", {
						 
							name = "unique_action_name",
							duration = 2000,
							label = "Farming is being done ....",
							useWhileDead = false,
							canCancel = true,
							controlDisables = {
								disableMovement = true,
...
Citizen.CreateThread(function()	
	...
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			if GetDistanceBetweenCoords(coords, Config.zonesfarm.cow.coords, true) < 2 then
...
function ValidateCoord(Coord)
	if spawnedProps > 0 then
		...
		if GetDistanceBetweenCoords(Coord, Config.zonesfarm.farm.coords, false) > 50 then
			validate = false
		end
...
function GeneratePropCoords()
	while true do
		...
		CoordX = Config.zonesfarm.farm.coords.x + modX
		CoordY = Config.zonesfarm.farm.coords.y + modY
