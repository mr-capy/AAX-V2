$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://esx_garage/escape', JSON.stringify({}));
		}
	};
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://esx_garage/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://esx_garage/unimpound', JSON.stringify(plate));
	});

	$('table').on('click', '.unlock', function () {
		var plate = $(this).parent().parent().find('#plate').text();
		$.post('http://esx_garage/unlock', JSON.stringify(plate));
	});
	...
	$('#cancel, #exit').click(function (event) {
		$.post('http://esx_garage/escape', null);
	});
