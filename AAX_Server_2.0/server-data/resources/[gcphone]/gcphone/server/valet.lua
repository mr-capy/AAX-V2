...
ESX.RegisterServerCallback('gcPhone:getCars', function(source, cb)
    ...
    MySQL.Async.fetchAll("SELECT plate, vehicle, stored FROM owned_vehicles WHERE owner = @cid and type = @type AND owned_vehicles.plate NOT IN (SELECT plate from h_impounded_vehicles)" , {
