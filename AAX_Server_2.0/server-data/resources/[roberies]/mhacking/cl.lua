		...
		if Vdist(GetEntityCoords(PlayerPedId(), true), Terminal.x, Terminal.y, Terminal.z) <= 0.5 then
			if VaultDoor ~= nil and VaultDoor ~= 0 then
				...
				if not CodeEnteredCorrect and CodeNeeded then
					...
					TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":Alert:Someone trying to Hack Main Bank Security.", PlayerCoords, {PlayerCoords = { x = 241.68, y = 220.97, z = 106.29 },})
					...
					if success then
						...
						TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":Alert:Someone Successfully Hack Main Bank Security.", PlayerCoords, {PlayerCoords = { x = 241.68, y = 220.97, z = 106.29 },})
				
					else
						print('Failure')
						TriggerEvent('mhacking:hide')
						TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":Alert:Someone Failed To hack Main Bank Security.", PlayerCoords, {PlayerCoords = { x = 241.68, y = 220.97, z = 106.29 },})
