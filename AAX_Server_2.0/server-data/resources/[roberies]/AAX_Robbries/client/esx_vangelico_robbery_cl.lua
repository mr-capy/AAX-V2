...
function DrawText3D_jewel(x, y, z, text, scale)
...
RegisterNetEvent("mt:missiontext_jewel")
AddEventHandler("mt:missiontext_jewel", function(text, time)
    ClearPrints()
...
function loadAnimDict_jewel( dict )  
...
RegisterNetEvent('esx_vangelico_robbery:currentlyrobbing_jewel')
AddEventHandler('esx_vangelico_robbery:currentlyrobbing_jewel', function(robb)
	holdingup = true
	store = robb
end)

RegisterNetEvent('esx_vangelico_robbery:killblip_jewel')
AddEventHandler('esx_vangelico_robbery:killblip_jewel', function()
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_vangelico_robbery:setblip_jewel')
AddEventHandler('esx_vangelico_robbery:setblip_jewel', function(position)
...
RegisterNetEvent('esx_vangelico_robbery:toofarlocal_jewel')
AddEventHandler('esx_vangelico_robbery:toofarlocal_jewel', function(robb)
	holdingup = false
...
RegisterNetEvent('esx_vangelico_robbery:robberycomplete_jewel')
AddEventHandler('esx_vangelico_robbery:robberycomplete_jewel', function(robb)
	holdingup = false
...
function drawTxt_jewel(x, y, scale, text, red, green, blue, alpha)
	SetTextFont(4)
	SetTextProportional(1)
...
Citizen.CreateThread(function()
	...
	while true do
						...
						if IsPedShooting(GetPlayerPed(-1)) then
							if Config.NeedBag then
							    if borsa == 40 or borsa == 41 or borsa == 44 or borsa == 45 then
							        ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio_jewel', function(CopsConnected)
								        if CopsConnected >= Config.RequiredCopsRobJewelry then
							                TriggerServerEvent('esx_vangelico_robbery:rob_jewel', k)
									        PlaySoundFromCoord(soundid, "VEHICLES_HORNS_AMBULANCE_WARNING", pos2.x, pos2.y, pos2.z)
								        else
									        TriggerEvent('esx:showNotification', _U('min_two_police') .. Config.RequiredCopsRobJewelry .. _U('min_two_police2'))
								        end
							        end)		
						        else
		...
		if holdingup then
			drawTxt_jewel(0.3, 1.4, 0.45, _U('smash_case') .. ' :~r~ ' .. vetrineRotte .. '/' .. Config.MaxWindows, 185, 185, 185, 255)
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					DrawText3D_jewel(v.x, v.y, v.z, '~w~[~g~E~w~] ' .. _U('press_to_collect'), 0.6)
					if IsControlJustPressed(0, 38) then
					    ...
					    loadAnimDict_jewel( "missheist_jewel" ) 
						TaskPlayAnim(GetPlayerPed(-1), "missheist_jewel", "smash_case", 8.0, 1.0, -1, 2, 0, 0, 0, 0 ) 
						TriggerEvent("mt:missiontext_jewel", _U('collectinprogress'), 3000)
						...
						if vetrineRotte == Config.MaxWindows then 
							...
							TriggerServerEvent('esx_vangelico_robbery:endrob_jewel', store)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				TriggerServerEvent('esx_vangelico_robbery:toofar_jewel', store)
				holdingup = false
				for i,v in pairs(vetrine) do 
					v.isOpen = false
					vetrineRotte = 0
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity >= Config.MaxJewelsSell then
								ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsSellJewelry then
										FreezeEntityPosition(playerPed, true)
										TriggerEvent('mt:missiontext_jewel', _U('goldsell'), 10000)
										Wait(10000)
										...
										blip = false
										TriggerEvent('esx:showNotification', _U('copsforsell') .. Config.RequiredCopsSellJewelry .. _U('copsforsell2'))
									end
								end)
							else
								blip = false
