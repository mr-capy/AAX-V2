                ...
                if not seatbeltIsOn then
                    ...
                    if(beltWarningSet == false) then
                        if(currSpeed > 1 or currSpeed < -1) then
                            beltWarningSet = true
                            TriggerEvent("mole_notifications:SendNotification", {text = "Remember to put on your belt!.", type = "info", timeout = 1400})
			...
			if IsControlJustReleased(0,  Config.vehicle.keys.seatbelt) then
				...
				if(PedVehIsCar)then
					...
					if seatbeltIsOn then
						...
						TriggerEvent("mole_notifications:SendNotification", {text = "Belt on", type = "success", timeout = 1400})
						TriggerServerEvent('esx_mole_misiones:PlayOnSource','buckle', 0.9)
					else
						TriggerEvent("mole_notifications:SendNotification", {text = "Belt removed", type = "error", timeout = 1400})
