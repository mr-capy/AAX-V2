			...
			for k in pairs(locations) do
				if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1)), false) < 2 then			
						TriggerServerEvent('esx_drugs:get')
						TriggerEvent('esx_drugs:new', k)
					end
...
local making = true
	...
	ESX.TriggerServerCallback('esx_drugs:process', function(output)
			making = output
		end)
...
RegisterNetEvent('esx_drugs:start')
...
RegisterNetEvent('esx_drugs:new')
AddEventHandler('esx_drugs:new', function(id)
	local set = false
	Citizen.Wait(10)
...
RegisterNetEvent('esx_drugs:message')
AddEventHandler('esx_drugs:message', function(message)
