...
RegisterNetEvent('esx_drugs:stop')
AddEventHandler('esx_drugs:stop', function()
	started = false
	DisplayHelpText("~r~Production stopped...")
	FreezeEntityPosition(LastCar, false)
end)
RegisterNetEvent('esx_drugs:stopfreeze')
AddEventHandler('esx_drugs:stopfreeze', function(id)
	FreezeEntityPosition(id, false)
end)
RegisterNetEvent('esx_drugs:notify')
AddEventHandler('esx_drugs:notify', function(message)
	ESX.ShowNotification(message)
end)

RegisterNetEvent('esx_drugs:startprod')
AddEventHandler('esx_drugs:startprod', function()
...
RegisterNetEvent('esx_drugs:blowup')
AddEventHandler('esx_drugs:blowup', function(posx, posy, posz)
...
RegisterNetEvent('esx_drugs:smoke')
AddEventHandler('esx_drugs:smoke', function(posx, posy, posz, bool)
...
RegisterNetEvent('esx_drugs:drugged')
AddEventHandler('esx_drugs:drugged', function()
						...
						if IsControlJustReleased(0, Keys['G']) then
							if pos.y >= 3500 then
								if IsVehicleSeatFree(CurrentVehicle, 3) then
									TriggerServerEvent('esx_drugs:start')	
									progress = 0
									pause = false
									selection = 0
									quality = 0
			...
			if progress < 96 then
				...
				if progress > 22 and progress < 24 then
					...
					if selection == 2 then
						...
						TriggerServerEvent('esx_drugs:blow', pos.x, pos.y, pos.z)
				...
				if progress > 30 and progress < 32 then
					...
					if selection == 2 then
						...
						pause = false
						TriggerEvent('esx_drugs:drugged')
					end
					if selection == 3 then
						print("Slected 3")
				...
				if IsPedInAnyVehicle(playerPed) then
					TriggerServerEvent('esx_drugs:make', pos.x,pos.y,pos.z)
					if pause == false then
						selection = 0
						quality = quality + 1
				...
				TriggerServerEvent('esx_drugs:finish', quality)
				FreezeEntityPosition(LastCar, false)
			end	
